<script lang="ts">
  ...
  function protegerRotas(u: any) {
    ...
    // Bloqueia Atendente de sair da sua rota
    if (u.nivel === 'atendente' && !path.startsWith('/atendimento') && path !== '/') {
      window.location.href = '/atendimento/status';
    }
    // Bloqueia Líder de entrar no ADM
    if (u.nivel === 'supervisor' && path.startsWith('/adm')) {
      ...
      <div class="nav-container">
        ...
        <div class="nav-links">
          ...
          {#if user.nivel === 'diretoria'}
            <div class="dropdown">
              <button class="dropbtn adm" class:active={$page.url.pathname.includes('/adm')}>ADM ▼</button>
              <div class="dropdown-content">
...
<style>
  ...
  .badge-matriz { background: #0f172a; color: white; font-size: 0.6rem; font-weight: 900; padding: 2px 6px; border-radius: 3px; }

  .nav-links { display: flex; gap: 10px; }
  .nav-links a, .dropbtn { 
    text-decoration: none; color: #64748b; font-size: 0.8rem; font-weight: 800; 
    padding: 10px 15px; border-radius: 6px; border: none; background: none; cursor: pointer;
  }
  ...
  .dropdown-content { 
    ...
    min-width: 180px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;
  }
  .dropdown:hover .dropdown-content { display: block; }
  .dropdown-content a { display: block; padding: 12px 15px; border-bottom: 1px solid #f1f5f9; border-radius: 0; font-size: 0.75rem; color: #475569; }
  .dropdown-content a:hover { background: #f8fafc; color: #2563eb; }

  .dropbtn.adm { color: #854d0e; border: 1px solid #fef08a; background: #fefce8; }

  .user-info { display: flex; align-items: center; gap: 15px; padding-left: 15px; border-left: 1px solid #e2e8f0; }
  .user-info .text { text-align: right; line-height: 1.1; }
  .user-info strong { display: block; font-size: 0.85rem; color: #0f172a; }
  .user-info span { font-size: 0.65rem; color: #94a3b8; }

  .logout { background: #fff1f2; color: #e11d48; border: 1px solid #fda4af; padding: 5px 10px; border-radius: 5px; font-size: 0.7rem; font-weight: 800; cursor: pointer; }
